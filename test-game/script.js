    ...
    fetch(`https://raw.githubusercontent.com/FranManre/FranManre.github.io/main/test-game/exams/${examName}`)
        ...
        .then(exam => {
            if (!Array.isArray(exam)) {
                throw new Error("El archivo JSON no tiene el formato correcto. Se esperaba un array de preguntas.");
            }
            currentExam = exam;
...
function showResults() {
    ...
    document.getElementById("results").innerHTML = `<h2>Resultados</h2> <p>Acertadas: ${correctAnswers}/${currentExam.length} (${percentage.toFixed(2)}%)</p>`;
